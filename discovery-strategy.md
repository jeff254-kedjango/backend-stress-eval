...
> **Working rules — read first.** All work on this task must follow the thirteen rules in [`rules.md`](./rules.md) at all times, whether or not they are restated in a given prompt. `rules.md` is canonical and must be referenced consistently through every plan, implementation, review, and handoff. Any deviation must be called out and justified in the change itself.
>
> **Next-round direction:** [`upgrade-plan.md`](./upgrade-plan.md) is the authoritative reviewer-bar roadmap (anchored 2026-08-06, supersedes the 2026-08-05 C+ → A version). Read it before proposing new invariants, layers, or candidates — it names which axes are saturated (do not extend), which are unsaturated (target these), and the three sourcing gates (repro affidavit, difficulty gate, own-words writeup) that every candidate must clear before packaging.
>
> **2026-08-06 amendment — read before treating the historical text below as prescriptive:** The strategy captured here (harvest → filter → package) produced tasks that failed external review on three axes: (a) difficulty ceiling < 1 hour of model work; (b) divergence absent (models converged); (c) narratives inherited from issue threads rather than personally reproduced — the reviewer specifically found the dramatiq #431 report described behaviour that does not occur at its pinned commit. The document below is retained as historical record of how the strategy evolved and is still valid on the parts it got right (four qualities, saturated-vs-unsaturated axes, prompt-only tasks). It is **superseded on sourcing**: candidates now originate from personal on-bench reproduction, enforced by Gates 1-3 in `upgrade-plan.md` §4 and Rules 11-13 in `rules.md`. Where the text below discusses harvesting issue threads for candidate narratives, treat that as *pointer-only* — issues may suggest a pin to try, but the narrative is always written from the author's own bench transcript.
